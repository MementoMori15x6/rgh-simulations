...
| **Layer 1 (Diffusion-Limited Energy Scarcity)** | New architecture — flat independent income replaced with a genuine shared, diffusing resource. Brian's Brain shows a real, dramatic interior complexity peak (a sharp extinction threshold, then complexity roughly doubling at an intermediate diffusion rate before smoothly declining) — checked directly against GoL and Day & Night, and confirmed rule-dependent, not universal: neither shows the same dramatic signature, since neither requires spatial energy concentration to survive at all the way Brian's Brain does. Plausibly replicates a known excitable-medium phenomenon (coherent wave sweet-spots in reaction-diffusion systems) rather than constituting a new principle. Single-rule only — the genuine ecological (multi-rule competition) extension remains unbuilt. Not yet part of the main paper. |
...
layer1-diffusive-scarcity/
  README.md                          <- real interior complexity peak, confirmed rule-dependent not universal
  stage1_diffusive_brians_brain.py    <- diffusive Landauer gate, single rule (Brian's Brain), the real peak
  stage2_diffusive_rule_comparison.py <- same mechanism generalized to GoL/Day & Night, confirms rule-dependence
